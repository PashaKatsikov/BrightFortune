...
//   5. Upsert the recipe's identifying tuple into the portfolio
//      registry (keyed on applicationId, so a re-mint of the same
//      project replaces its own row instead of colliding with it)
...
Future<void> main(List<String> argv) async {
  ...
  if (!dryRun) {
    ...
    if (portfolioPath.isNotEmpty) {
      ...
      final int existing = portfolio.indexWhere((Map<String, Object?> p) =>
          p['applicationId'] == entry['applicationId']);
      if (existing >= 0) {
      ...
      _log('portfolio:  ${existing >= 0 ? "replaced" : "added"} entry '
          '($portfolioPath)');
    }
  }
  _log('\nOK — forge complete.');
...
void _writeVeiledBytes(Mutation m, {required bool dryRun}) {
  ...
  final Map<String, String> enhancers = _loadEnhancerBodies(m);
  ...
  final Map<String, String> plaintext = <String, String>{
    ...
    // JS bodies come from the per-project variant pack in
    // tool/forge/jsvariants/, selected by webView.enhancers. A slot
    // left out of that list ships empty and WebScripts skips it.
    '_jsSafeAreaScript': enhancers['safeArea'] ?? '',
    '_jsKeyboardScript': enhancers['keyboard'] ?? '',
    '_jsAutoplayScript': enhancers['autoplay'] ?? '',
    '_jsViewportTintScript': enhancers['viewportTint'] ?? '',
    // Not an install-time body: a call template the host evaluates on
    // every keyboard metric change. See jsvariants/keyboard_bridge.js.
    '_jsKeyboardBridge': enhancers['keyboardBridge'] ?? '',
  };
...
/// Known enhancer slots. Adding a behaviour means adding it here, to the
/// plaintext map in [_writeVeiledBytes], to the footer accessors, and to
/// `WebScripts._bodies()` — in that order.
const Set<String> _enhancerSlots = <String>{
  'safeArea',
  'keyboard',
  'autoplay',
  'viewportTint',
...
/// Reads the enabled enhancer bodies off disk and condenses them.
///
/// `webView.enhancers` lists which behaviours ship; `webView.enhancerBodies`
/// maps each name to a file under `tool/forge/jsvariants/`. Dropping a
/// canonical behaviour and adding a project-specific one is the point —
/// see `.cursor/rules/relay_forge.md` §5.
Map<String, String> _loadEnhancerBodies(Mutation m) {
  final List<String> enabled =
      (m.webView['enhancers'] as List<Object?>? ?? const <Object?>[])
          .whereType<String>()
          .toList();

  for (final String name in enabled) {
    if (!_enhancerSlots.contains(name)) {
      _bail('unknown enhancer "$name" '
          '(known slots: ${_enhancerSlots.join(", ")})');
    }
  }

  final Map<String, Object?> bodies =
      m.webView['enhancerBodies'] as Map<String, Object?>? ?? const {};
  final Map<String, String> out = <String, String>{};

  for (final String name in enabled) {
    final Object? path = bodies[name];
    if (path is! String || path.isEmpty) {
      _log('WARNING: enhancer "$name" is enabled but has no body file — '
          'the slot ships empty');
      continue;
    }
    final File file = File(path);
    if (!file.existsSync()) {
      _bail('enhancer body not found: $path');
    }
    final String body = _condenseJs(file.readAsStringSync());
    if (body.isEmpty) {
      _bail('enhancer body $path condensed to nothing');
    }
    out[name] = body;
    _log('enhancer:   $name ← $path (${body.length} bytes)');
  }

  for (final String slot in _enhancerSlots) {
    if (!enabled.contains(slot)) {
      _log('enhancer:   $slot — omitted from this project');
    }
  }
  return out;
}

/// Drops comment-only lines and indentation, keeping every newline so
/// automatic semicolon insertion behaves exactly as in the source file.
///
/// Deliberately does NOT strip trailing `//` comments or `/* */` blocks —
/// that needs a real tokenizer to stay safe around string literals. Keep
/// variant files free of both.
String _condenseJs(String source) {
...
String unlockJsViewportTintScript() => reveal(_jsViewportTintScript);
...
void _checkCollisions(
  ...
  // A re-mint of THIS project is not a collision with itself. The
  // registry is keyed on applicationId, so its own previous entry is
  // skipped here and replaced (not appended) after a successful run.
  final List<Map<String, Object?>> siblings = portfolio
      .where((Map<String, Object?> p) =>
          p['applicationId'] != entry['applicationId'])
      .toList();
  if (siblings.isEmpty) return;

  for (final Map<String, Object?> prev in siblings) {
  ...
  final Map<String, Object?> last = siblings.last;
