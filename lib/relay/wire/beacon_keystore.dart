...
class BeaconKeystore {
  ...
  // ── Cold push-tap de-duplication ────────────────────────
  // A push URL is NEVER persisted: it belongs to the launch that was
  // started by the tap and to no other. What we do persist is the id
  // of the last tap we already honoured, because Android hands the
  // very same launch intent back when the task is restored from the
  // launcher — without this the app would keep re-opening the push
  // URL on every restart instead of the URL the config returned.
  bool pushAlreadyOpened(String id) => _prefs.getString(_kPushSeen) == id;

  Future<void> markPushOpened(String id) => _prefs.setString(_kPushSeen, id);
