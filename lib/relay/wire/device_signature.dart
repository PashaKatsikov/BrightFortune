...
class DeviceSignature {
  ...
  /// Android API level, or 0 before [prime] runs and on other
  /// platforms. Read while assembling the UA, so it costs nothing
  /// extra; the portal uses it to tell which side owns the keyboard
  /// geometry.
  static int get androidSdk => _sdk;
  ...
  // Android UA assembly
  // ─────────────────────────────────────────────────────────
  // GAME THEME CATEGORY: other — tower defense, neither a slot nor a
  //                      crash title. The operator confirmed the app
  //                      identity is not needed in the UA, so the
  //                      appid/appname suffix is intentionally absent.
